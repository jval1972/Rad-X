...
begin
  ...
  begin
    D_AddFile(raddat);
    radix_crc32 := strupper(GetCRC32(raddat));
  end
  else
    I_Warning('D_AddRadixDAT(): Radix data file "%s" not found.'#13#10, [tmpdat]);
end;
...
var
  ...
  wad: string;
begin
  gamedirectorystring := 'RADIX,RADIX1';
...
{$ENDIF}
  ...
  if (p <> 0) and (p < myargc - 1) then
  begin
    sprintf(filename, M_SaveFileName(SAVEGAMENAME) + '%s.sav', [myargv[p + 1][1]]);
    G_LoadGame(filename);
  end;
...
begin
  ...
  W_ShutDown;
  printf('RX_ShutDownRadixAlias: Shut down RADIX alias entries.'#13#10);
